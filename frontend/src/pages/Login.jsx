    ...
    <div className="min-h-[calc(100vh-64px)] grid place-items-center p-6">
      <form onSubmit={submit} className={`w-full max-w-sm space-y-4 p-6 border rounded-2xl backdrop-blur shadow-lg transition-all duration-300 hover:shadow-xl hover:-translate-y-0.5 ${theme === "dark" ? "bg-slate-900/70 border-slate-800" : "bg-white/80 border-slate-200"}`}>
        <div className="space-y-2 text-center">
          ...
          <div>
            <h1 className={`text-2xl font-semibold tracking-tight ${theme === "dark" ? "text-gray-100" : "text-slate-900"}`}>Mini Zapier</h1>
            <p className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-slate-600"}`}>Login to your account</p>
          </div>
        </div>
        ...
        <div className={`text-xs text-center ${theme === "dark" ? "text-gray-400" : "text-slate-500"}`}>By signing in you agree to our Terms and Privacy Policy.</div>
        <div className={`text-sm text-center ${theme === "dark" ? "text-gray-400" : "text-slate-600"}`}>Don't have an account? <Link to="/signup" className="text-cyan-400 underline">Sign up</Link></div>
